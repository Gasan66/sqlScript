...
begin tran plan2020
    ...
    from PurchaseRequests pr
        ...
        left join ProtocolCzoItems pci on ac.Id = pci.AuctionCycleId
    where pci.Protocol_Id = 395 or pci.Protocol_Id = 386

-- select pr.Id, pc.Status
-- from PurchaseRequests pr
-- left join AuctionCycles AC on pr.Id = AC.RequestId
-- left join ProtocolCzoItems PCI on AC.Id = PCI.AuctionCycleId
-- left join ProtocolCzoes PC on pci.Protocol_Id = PC.Id
-- where pci.Protocol_Id = 386
--
-- select PurchasePlanYear
-- from PurchaseRequests
-- where id = 21000

-- update PurchaseRequests set PurchasePlanYear  = '2019' where Id = 21000;

USE ASUZD
DECLARE @PurchasePalnYear int = 2020
DECLARE @CustomerOrganisation nvarchar(100) = N'АО "ЕЭСК"'
--     N'ОАО "МРСК Урала"'
;
WITH IPRData AS (
...
SELECT
  ...
  LEFT JOIN [dbo].[LongTermPurschasePayments] AS LTP_plus_1Y ON LTP.AuctionCycleId = LTP_plus_1Y.AuctionCycleId AND LTP_plus_1Y.PaymentYear = YEAR(GETDATE()) + 1
  LEFT JOIN [dbo].[LongTermPurschasePayments] AS LTP_plus_2Y ON LTP.AuctionCycleId = LTP_plus_2Y.AuctionCycleId AND LTP_plus_2Y.PaymentYear = YEAR(GETDATE()) + 2
  LEFT JOIN [dbo].[LongTermPurschasePayments] AS LTP_plus_3Y ON LTP.AuctionCycleId = LTP_plus_3Y.AuctionCycleId AND LTP_plus_3Y.PaymentYear = YEAR(GETDATE()) + 3
  WHERE LTP.PaymentYear = @PurchasePalnYear
...
WHERE PR_RH_AC.[PurchasePlanYear] = @PurchasePalnYear
AND ORG.[Name] = @CustomerOrganisation
