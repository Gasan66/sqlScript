SELECT
			...
			,(CASE WHEN (
						...
						AND (SELECT COUNT(*) FROM [dbo].[AuctionCycles] AS AC WHERE AC.[RequestId] = 20846 /*UPR.[RequestId]*/) > 1
				...
				THEN (SELECT TOP 1 [Status] FROM [dbo].[AuctionCycles] AS AC WHERE AC.RequestId =20846 UPR.[RequestId]) --N'Нужно брать Status из предыдущего аукционного цикла'
